...
class generate_sales_report:
    ...
    def find_min_max_price_item(self) -> Tuple[Tuple[str, float], Tuple[str, float]]:
        #vMin = [y for y in self.sales_data ]
        vFmax = (lambda x,y: x if x[1] > y[1] else y)
        vFPrecioMaximo = lambda arr: reduce(vFmax,arr)
        vResMaximo = vFPrecioMaximo(self.sales_data)
        vFmin = (lambda x,y: x if x[1] < y[1] else y)
        vFPrecioMinimo = lambda arr: reduce(vFmin,arr)
        vResMinimo = vFPrecioMinimo(self.sales_data)
        vRes = ([vResMaximo[0],vResMaximo[1]],[vResMinimo[0],vResMinimo[1]])
        return vRes
    ...
    def ejecutar(self):
        ...
        vLista= self.find_min_max_price_item()
        vCad += 'Artículo Más Barato:'+ vLista[1][0] + ' a ' + str(vLista[1][1])+ '\n'
        vCad += 'Artículo Más Caro:'+ vLista[0][0] + ' a ' + str(vLista[0][1])+ '\n'
        vCad +='Total de Ventas: ' + self.currency + str(vTotalC) + '\n'
        print(vCad)
